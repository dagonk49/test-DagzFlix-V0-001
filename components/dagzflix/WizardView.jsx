  ...
  const discover = async () => {
    ...
    try {
      const r = await api('wizard/discover', {
        method: 'POST',
        body: JSON.stringify({ mood, era, duration, mediaType: isTV ? 'tv' : 'movie' }),
      });
          ...
          <motion.div key="w4" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
            {loading ? (
              ...
              <div>
                ...
                <div className="text-center mt-8"><Button data-testid="wizard-reset" onClick={reset} variant="outline" className="border-white/10 text-gray-300 rounded-xl"><Wand2 className="w-4 h-4 mr-2" />Relancer le Magicien</Button></div>
              </div>
            ) : (
              <div className="text-center py-20">
